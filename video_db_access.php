	...
	if(isset($_REQUEST["operation"])){
        if($_REQUEST["operation"] == $OP_GETBYID){
            try{
                ...
                $req = $connexion->prepare("SELECT * FROM video WHERE id = $id");
                $req->execute();
                
                //si video trouvée, on envoie le résultat
            ...
            try{
                ...
                print($OP_GETALL . "%");
                $connexion = connexionPDO();
                $req = $connexion->prepare("SELECT * FROM video ORDER BY id");
                $req->execute();
            ...
            try{
				...
				print($OP_ADD . "%");
				$connexion = connexionPDO();
				$requete = "INSERT INTO video(chemin)";
            ...
            try{
                ...
                print($OP_REMOVE."%");
                $connexion = connexionPDO();
                $req = $connexion->prepare("DELETE FROM video WHERE id = $id");
                $req->execute();
            ...
            try{
                ...
                $connexion = connexionPDO();
                $req = $connexion->prepare("SELECT * FROM video WHERE id IN (SELECT id_video FROM camera_video WHERE id_camera = $id_camera) ORDER BY id");
                $req->execute();
            ...
            try{
				...
				$requete = "INSERT INTO camera_video(id_camera,id_video)";
				$requete .= "VALUES ('$id_camera','$id_video');";
				$req = $connexion->prepare($requete);
				$req->execute();
